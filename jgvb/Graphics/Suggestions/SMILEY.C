/*Write a program in C to  draw a smiley face.
Use different colors for eyes, nose and mouth.
Write your Roll number, Name on screen.*/
...
int main()
{
    ...
    draw_xy();
    for(i=r;i<=r+3;i++)
    circle_draw(xc,yc,i,c_line); //Face Outline
    circle_draw(xc-25,yc+20,12,1);    //left eye
    for(i=5;i>=1;i--)
    circle_draw(xc-25,yc+14,i,8);    //left pupil
    circle_draw(xc+25,yc+20,12,1);   //right eye
    for(i=5;i>=1;i--)
    circle_draw(xc+25,yc+14,i,8);    //right pupil
    for(i=7;i>=1;i--)
    circle_draw(xc,yc,i,6);       //nose
    for(i=25;i>=22;i--)
    {
    circle_draw(xc-r-10,yc+50,i,1);     //left ear
    circle_draw(xc+r+10,yc+50,i,1);     //right ear
    }
    for(i=20;i>=1;i--)
    halfcircle_draw(xc,yc-20,i,3);      //mouth
